...
int getSelection(char *title, char *items[], int count) {
    ...
    if (scanf("%d", &userChoice) != 1) return -1;
    if (userChoice < 1 || userChoice > count) return -1;
    
    return userChoice - 1;
}
int main() {
    // Descriptive index names
    int continentPos, countryPos, statePos, cityPos;
    ...
    continentPos = getSelection("SELECT YOUR CONTINENT", continents, 7);
    if (continentPos == -1) { 
        printf("Invalid!\n"); 
        return 0; 
    }
    ...
    // 2. Country Selection
    char *countryTitle = (continentPos == 1) ? "SELECT RESEARCH STATION" : "SELECT YOUR COUNTRY";
    countryPos = getSelection(countryTitle, countries[continentPos], 10);
    if (countryPos == -1) return 0;

    // Antarctica special case
    if (continentPos == 1) {
        printf("\n✅ SUCCESS: DESTINATION LOCKED!\n");
        printf(">> Continent: %s\n", continents[continentPos]);
        printf(">> Station:   %s\n", countries[continentPos][countryPos]);
        return 0;
    }

    // 3. State Selection (3D Array Access)
    statePos = getSelection("SELECT STATE / REGION", states[continentPos][countryPos], 10);
    if (statePos == -1) return 0;

    // 4. City Selection (4D Array Access) 
    // cities[continent][country][state][city]
    cityPos = getSelection("SELECT CITY / AREA", cities[continentPos][countryPos][statePos], 10);
    if (cityPos == -1) return 0;

    // FINAL OUTPUT (Formatting preserved)
    printf("\n✅ SUCCESS: DESTINATION LOCKED!\n");
    printf("--------------------------------------------------\n");
    printf("🗺️  TRAVEL DETAILS:\n");
    printf(">> Continent : %s\n", continents[continentPos]);
    printf(">> Country   : %s\n", countries[continentPos][countryPos]);
    printf(">> State     : %s\n", states[continentPos][countryPos][statePos]);
    printf(">> City      : %s\n", cities[continentPos][countryPos][statePos][cityPos]);
    printf("--------------------------------------------------\n");
